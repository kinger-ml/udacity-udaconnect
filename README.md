...
### Steps
#### Setup the message queue with topic location-grpc-data
1. `helm repo add bitnami https://charts.bitnami.com/bitnami` - Add Kafka packaged by bitnami
2. `helm install udaconnect-kafka bitnami/kafka  --kubeconfig <path_to_k3s.yaml>` - Installs Message Queue udaconnect-kafka
3. Once above kafka pod is up and running, create a kafka topic to be used by producer and consumer services
`kubectl exec -it <pod_name> --kafka-topics.sh --create --bootstrap-server <bootstrap-server> \ --topic location-grpc-data`
...
5. `kubectl apply -f deployment/udaconnect-api-persons.yaml` - Set up the service and deployment for the persons microservice
6. `kubectl apply -f deployment/udaconnect-api-connections.yaml` - Set up the service and deployment for the connections microservice
7. `kubectl apply -f deployment/udaconnect-location-consumer.yaml` - Set up the service and deployment for the location consumer microservice
8. `kubectl apply -f deployment/udaconnect-location-producer.yaml` - Set up the service and deployment for the location producer
...
* `http://localhost:30005/` - OpenAPI Documentation for connections API
* `http://localhost:30005/api/` - Base path for connections API
